...
app.post('/api/login', async (req:Request, res:Response) => {
    ...
    const accessToken=jwt.sign({email:email,id:customer.customer_id},SECRET_KEY_ACCESS,{expiresIn:Number(ACCESS_TOKEN_EXPIRES)});
    const refreshToken=jwt.sign({email:email,id:customer.customer_id},SECRET_KEY_REFRESH,{expiresIn:Number(REFRESH_TOKEN_EXPIRES)});
...
app.post('/api/refresh', async (req:Request, res:Response) => {
    ...
    try {
        ...
        const accessToken = jwt.sign({ email: customer.email, id: customer.id }, SECRET_KEY_ACCESS, { expiresIn: ACCESS_TOKEN_EXPIRES });
